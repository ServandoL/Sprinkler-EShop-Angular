...
export class StarRatingControlComponent extends StarRating implements ControlValueAccessor {
  @Output()
  ...
  onKeyDown(event: KeyboardEvent) {
    ...
    const handleDigits = (eventCode: string): void => {
      const dStr = 'Digit';
      const digit: number = parseInt(eventCode.substr(dStr.length, eventCode.length - 1), 10);
      this.rating = digit;
    };
